#include "PEngine/graphics/static_sprite_renderer.h"
...
namespace pear { namespace graphics {
	...
	StaticSpriteRenderer::~StaticSpriteRenderer()
	{
		...
		for( unsigned int i = 0; i < m_RenderedSprites.size(); ++i )
			m_RenderedSprites.pop_back();
	}
	
	void StaticSpriteRenderer::submit(Renderable2D* renderable)
	{
		renderable->renderer = this;
		
		if( renderable->renderableType == Renderable2D::RenderableType::SPRITE )
		{
			m_Sprites.push_back( (Sprite*)renderable );
		}
		else if( renderable->renderableType == Renderable2D::RenderableType::GROUP )
		{
			for( unsigned int i = 0; i < ((Group*)renderable)->renderables.size(); ++i )
			{
	...
	void StaticSpriteRenderer::erase( Renderable2D* renderable )
	{
	...
	{
		m_Shader.compileShaders( shaders::DefaultStaticSpriteShader32_SRC.c_str() );
	...
	void StaticSpriteRenderer::sendDataToGPU()
	{
		...
		for( int i = 0; i < m_RenderedSprites.size(); i++ )
		{
			...
			vertexData[ x ].setUV( m_RenderedSprites[i]->uv.x, m_RenderedSprites[i]->uv.w );
			...
			vertexData[x+1].setUV( m_RenderedSprites[i]->uv.z, m_RenderedSprites[i]->uv.w );
			...
			vertexData[x+2].setUV( m_RenderedSprites[i]->uv.x, m_RenderedSprites[i]->uv.y );
			...
			vertexData[x+3].setUV( m_RenderedSprites[i]->uv.z, m_RenderedSprites[i]->uv.y );
