...
namespace pear { namespace graphics {
	...
	{
		animations.push_back( SS_Animation( texture->filename, nosh, nosv, first_sprite, last_sprite ) );
		startAnimation( 0 );
	}
	
	void SpriteSheet::addAnimationWithSP( const char* animation_name )
	{
		animations.push_back( SS_Animation( texture->filename, animation_name ) );
		startAnimation( 0 );
	}
